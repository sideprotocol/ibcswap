...
func (s *InterchainswapTestSuite) TestCancelMultiDepositStatus() {
	...
	// swap message
	t.Run("cancel multi-asset deposit", func(t *testing.T) {
		...
		testCases := []struct {
			...
			{
				"cancel multi-deposit Assets",
				func() {
					packetSequence = 3
				},
				"cancel multi-deposit",
				true,
			},
		}
		...
		for _, tc := range testCases {
			...
			case "cancel multi-deposit":
				order := GetFirstOrderId(s, ctx, chainA, poolId)
				logger.CleanLog("orderId", order)
				msg := types.NewMsgCancelMultiAssetDeposit(
					channel.PortID,
					channel.ChannelID,
					chainAAddress,
					poolId,
			...
			// pool status log.
			if tc.msgType == "cancel multi-deposit" {
				_, err := s.QueryLatestMultiDepositOrder(ctx, chain, poolId, pool.SourceCreator)
				s.Require().Error(err)
