...
const (
	...
	flagExpectedCounterparty = "expected-counterparty"
)

// NewMakeSwapTxCmd returns the command to create a NewMsgMakeSwap transaction
func NewMakeSwapTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "make [src-channel] [amount] [receiving-address] [receiving-token-amount]",
		Short: "Make a tokens swap through IBC",
...
func NewTakeSwapTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "take [order-id] [sell-token-amount] [receiving-address]",
		Short: "take a tokens swap through IBC",
		Long: strings.TrimSpace(`Swap a token through IBC. Timeouts can be specified
as absolute or relative using the "absolute-timeouts" flag. Timeout height can be set by passing in the height string
in the form {revision}-{height} using the "packet-timeout-height" flag. Relative timeout height is added to the block
height queried from the latest consensus state corresponding to the counterparty channel. Relative timeout timestamp 
...
corresponding to the counterparty channel. Any timeout set to 0 is disabled.`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			sellToken, err := sdk.ParseCoinNormalized(args[1])
			...
			msg := types.NewMsgTakeSwap(
				orderId, sellToken,
				sender, receivingAddress,
				timeoutHeight, timeoutTimestamp, time.Now().UTC().Unix(),
			)
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
		},
...
//	cmd := &cobra.Command{
//		Use:   "cancel [src-channel] [amount] [receiving-address] [receiving-token-amount]",
//		Short: "Make a tokens swap through IBC",
//		Long: strings.TrimSpace(`Swap a token through IBC. Timeouts can be specified
//as absolute or relative using the "absolute-timeouts" flag. Timeout height can be set by passing in the height string
//in the form {revision}-{height} using the "packet-timeout-height" flag. Relative timeout height is added to the block
